...
ServerSocketBase::ServerSocketBase(const std::string& address, uint16_t port)
{
    ...
    if (bind(serverSocket, (struct sockaddr*)&localAddr, sizeof(struct sockaddr_in)) != 0)
        throw std::runtime_error("Failed bind server socket. (FD" + std::to_string(serverSocket) + ") (Port: " + std::to_string(port)
                                 + ")");
...
void ServerSocketBase::tcpAcceptLoop()
{
    ...
    for (;;) {
        ...
        if (clientSock > 0) {
            ...
            // Remove this throw?
            throw std::runtime_error("Fatal error in accept loop of server socket FD" + std::to_string(serverSocket)
                                     + " TCP accept failed.");
        }
    }
}
