...
{
    // The ability to use hostnames instead of ip was removed
    // std::string serverIp = convertToValidIpV4(address, port);
    std::string serverIp = address;
    ...
    if (connect(clientSock, (struct sockaddr*)&remoteAddr, sizeof(struct sockaddr_in)) != 0)
        throw std::runtime_error("Client socket connection to remote server has failed. Server: " + address
                                 + " Port: " + std::to_string(port) + " Ip: " + serverIp);
