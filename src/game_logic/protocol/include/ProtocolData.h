...
static std::unordered_map<std::string, MethodName> const MethodNameTable = {{"CONNECTED_OK", MethodName::CONNECTED_OK},
...
struct ProtocolData {
  public:
    explicit ProtocolData(const MethodName method, const std::unordered_map<std::string, std::string> data) : method(method), data(data) {}
 ...
 */
ProtocolData newProtocolMessage(MethodName method, std::initializer_list<std::pair<std::string, std::string>> dataFields);
