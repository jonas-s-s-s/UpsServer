...
void GameRoom::addUser(const std::string& username) {
    if (getUserCount() >= 2) {
        throw std::runtime_error("GameRoom::addUser: ERROR - Cannot add user " + username + " this game already has 2 players!");
    }
...
GameStateChange::GameStateChange(const GameState newState,
    ...
    : newState(newState), client1(std::move(client1)), client2(std::move(client2)) {}
