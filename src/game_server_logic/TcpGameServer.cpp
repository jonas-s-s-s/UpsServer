...
void TcpGameServer::_newClientConnectedHandler(int clientSocket) {
    ...
    _newClientsQueue.push(clientSocket);
}

TcpGameServer::TcpGameServer(const std::string &address, uint16_t port) : ServerSocketBase(address, port) {}

EventfdQueue<int> &TcpGameServer::getNewClientsQueue() {
    return _newClientsQueue;
}
